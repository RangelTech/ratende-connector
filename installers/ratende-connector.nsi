...
InstallDir "$PROGRAMFILES64\RAtende Connector"
...
Section "Instalar"
  ...
  WriteRegStr HKLM "SOFTWARE\Policies\Google\Chrome\ExtensionInstallForcelist" "1" "${EXT_ID};${UPDATE_URL}"
  ...
  IfFileExists "$PROGRAMFILES\Microsoft\Edge\Application\msedge.exe" 0 +2
    WriteRegStr HKLM "SOFTWARE\Policies\Microsoft\Edge\ExtensionInstallForcelist" "1" "${EXT_ID};${UPDATE_URL}"
  IfFileExists "$PROGRAMFILES32\Microsoft\Edge\Application\msedge.exe" 0 +2
    WriteRegStr HKLM "SOFTWARE\Policies\Microsoft\Edge\ExtensionInstallForcelist" "1" "${EXT_ID};${UPDATE_URL}"

  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\RAtendeConnector" "DisplayName" "RAtende Connector"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\RAtendeConnector" "UninstallString" "$INSTDIR\Desinstalar.exe"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\RAtendeConnector" "Publisher" "Rangel Tech"
...
Section "Uninstall"
  ...
  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\RAtendeConnector"
